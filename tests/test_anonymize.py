...
class TestAnonymizeWidget:
    def test_vehicle_name_is_replaced(self) -> None:
        result = anonymize_widget(make_widget_data())
        assert result["vehicle"]["name"] == VEHICLE_NAME

    def test_license_plate_is_replaced(self) -> None:
        result = anonymize_widget(make_widget_data())
        assert result["vehicle"]["licensePlate"] == LICENSE_PLATE

    def test_render_url_vin_is_replaced(self) -> None:
        result = anonymize_widget(make_widget_data())
        assert REAL_VIN not in result["vehicle"]["renderUrl"]
        assert "TMOCKAA0AA000000" in result["vehicle"]["renderUrl"]

    def test_render_url_structure_is_preserved(self) -> None:
        result = anonymize_widget(make_widget_data())
        assert result["vehicle"]["renderUrl"].startswith("https://skoda.com/render/")
        assert result["vehicle"]["renderUrl"].endswith("/image.png")

    def test_light_map_url_coordinates_are_replaced(self) -> None:
        result = anonymize_widget(make_widget_data())
        assert (
            f"latitude={REAL_LAT}&longitude={REAL_LON}"
            not in result["parkingPosition"]["maps"]["lightMapUrl"]
        )
        assert (
            f"latitude={LOCATION['latitude']}&longitude={LOCATION['longitude']}"
            in result["parkingPosition"]["maps"]["lightMapUrl"]
        )

    def test_light_map_url_other_params_are_preserved(self) -> None:
        result = anonymize_widget(make_widget_data())
        assert "bar=11" in result["parkingPosition"]["maps"]["lightMapUrl"]
        assert "foo=15" in result["parkingPosition"]["maps"]["lightMapUrl"]

    def test_gps_coordinates_are_replaced(self) -> None:
        result = anonymize_widget(make_widget_data())
        assert result["parkingPosition"]["gpsCoordinates"] == LOCATION

    def test_formatted_address_is_replaced(self) -> None:
        result = anonymize_widget(make_widget_data())
